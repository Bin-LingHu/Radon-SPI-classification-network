...
#from timm.models import create_model, safe_model_name, resume_checkpoint, load_checkpoint, convert_splitbn_model, convert_sync_batchnorm, model_parameters, set_fast_norm
...
def main():
    ...
    args.distributed = False  #默认关闭分布式模式
    if 'WORLD_SIZE' in os.environ:
        args.distributed = int(os.environ['WORLD_SIZE']) > 1 #如果环境变量WORLD_SIZE存在且大于1，则启用分布式训练
    args.device = 'cuda:1'
    args.world_size = 1
    ...
    # resolve AMP arguments based on PyTorch / Apex availability 混合精度标志：根据配置和环境，选择使用APEX AMP或原生Torch AMP
    use_amp = None
    if args.amp:
    ...
    # setup exponential moving average of model weights, SWA could be used here too 使用ModelEmaV2实现模型权重的指数移动平均
    model_ema = None
    if args.model_ema:
    ...
    # setup learning rate schedule and starting epoch 创建学习率调度器并确定总训练周期数
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        update_grad = (batch_idx + 1) % grad_accum_steps == 0 #梯度累积
            ...
            if update_grad:
                if args.clip_grad is not None:
                    utils.dispatch_clip_grad(
                        ...
                        value=args.clip_grad, mode=args.clip_mode) #梯度裁剪
                optimizer.step()

        if update_grad:
            optimizer.zero_grad()
            if model_ema is not None:
                model_ema.update(model) #EMA模型更新
